...
create table profiles (
  ...
  -- written to directly.
  -- When the angler last opened the Activity tab. One timestamp rather
  -- than a read flag per event: the events are derived, so there is no
  -- row to mark, and "everything since you last looked" is what the
  -- badge means anyway.
...
create table mini_leagues (
  ...
  season_id   uuid not null references seasons(id) on delete cascade,
  name        text not null,
  -- Cascade: owning a mini league must not make an angler undeletable,
  -- and mini_league_members already cascades on the same pair.
...
-- The policy above decides which ROWS an angler may write, and says nothing
-- about which COLUMNS. An UPDATE policy with only a USING clause applies that
-- same expression as its WITH CHECK, and `auth.uid() = id` still holds after
-- setting is_admin = true — so column privileges are what stop an angler
-- promoting themselves. See 20260807090000.
...
-- a row on every like — the events already exist in likes, comments, follows
-- and catch_reviews, and a second copy would be a second thing to keep in
-- step. Delete a comment and the notification about it goes with it, which is
-- free here and needs a cascade otherwise.
--
-- Not SECURITY DEFINER: every source table is already readable by the person
-- these events are about, so this runs as the caller and RLS keeps it honest
-- rather than the function re-implementing who may see what.
-- ===========================================================================
...
-- MINI LEAGUES
--
-- Private tables between anglers who know each other. Creating one is a paid
-- feature; being in one is not, and a paid member may invite anyone.
--
-- Scored on the national rule, not the divisional one: every verified fish of
-- the season counts, best 5 in summer and 3 in winter, whether or not the
-- angler pays and whenever they joined. There is no prize attached, so gating
-- it on membership would make it a worse version of the division, and gating
-- it on join date would mean a league started in August began with everyone
-- on zero.
--
-- Computed from catches directly rather than from scored_catches, which
-- inner-joins season_entries — a free member has no entry and so does not
-- appear in it at all, and free members are half the point of a mini league.
-- ===========================================================================
...
-- ============================================================================
-- 15. WITNESSED CATCHES
--
-- An angler nominates another paid member to vouch for a fish; the witness is
-- asked in their Activity feed and answers once. A confirmation is recorded
-- and displayed — it does NOT verify the catch or raise its evidence tier,
-- because two members who trust each other could otherwise wave anything
-- through, and the prize fund is on the other side of that.
-- ============================================================================
