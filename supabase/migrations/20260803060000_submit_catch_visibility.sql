-- submit_catch always inserted posts at the visibility column default
-- ('public'), so there was previously no way to log a catch as anything
-- else. Adds p_visibility, appended at the end so existing positional
-- callers (there's only the one client) keep working during the swap;
-- defaults to 'public' either way.
